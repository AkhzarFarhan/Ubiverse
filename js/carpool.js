...
const CarpoolModule = (function () {
  ...
  function renderList(rides) {
    ...
    rides.forEach(function (ride) {
      ...
      var allRiders = [{ name: ride.owner_name, phone: ride.owner_phone, role: 'Driver' }]
        .concat(passengers.map(function (p) { return { name: p.name, phone: p.phone, role: 'Rider' }; }));
      ...
      var whatsappMsg = encodeURIComponent(
        ...
        "📍 " + ride.masjid_name + "\n" +
        "🕒 " + formattedTime + "\n" +
        (ride.pickup_point ? "📌 Pickup: " + ride.pickup_point + "\n" : "") +
        "\n👥 Riders:\n" +
        allRiders.map(function (r) {
          return '• ' + r.name + (r.phone ? ' (+91 ' + r.phone + ')' : '') + (r.role === 'Driver' ? ' 🚗 Driver' : '');
        }).join('\n') +
        "\n\nPlease add everyone to a WhatsApp group for coordination. JazakAllahu Khairan! 🤲"
      );
      var whatsappUrl = 'https://api.whatsapp.com/send?phone=91' + ride.owner_phone + '&text=' + whatsappMsg;
      ...
      html +=
        '<div class="' + cardClass + '">' +
          ...
          '<div class="carpool-actions">' +
            ...
            (passengers.length > 0
              ? '<a href="' + whatsappUrl + '" target="_blank" class="btn btn-full carpool-whatsapp-btn">' +
