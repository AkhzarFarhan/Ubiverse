...
window.TexterModule = (function () {
  ...
  function render() {
    ...
    document.getElementById('texter-list').addEventListener('click', function (event) {
      if (event.target.closest('.texter-copy-note-btn')) {
        const btn = event.target.closest('.texter-copy-note-btn');
        copyNoteByIndex(btn.dataset.index);
      } else if (event.target.closest('.texter-delete-note-btn')) {
        const btn = event.target.closest('.texter-delete-note-btn');
        deleteNoteByIndex(btn.dataset.index);
      }
    });
  ...
  async function copyNoteByIndex(index) {
    ...
    async function deleteNoteByIndex(index) {
      clearAlert("texter-alert");
      const noteIndex = Number(index);
      if (!currentEntries.length || !Number.isInteger(noteIndex) || noteIndex < 0 || noteIndex >= currentEntries.length) {
        return;
      }
      ...
      const arr = currentEntries.slice();
      ...
      currentEntries.map(function (e, i) {
        return `<div class="entry-card">
          <div class="entry-meta">
            ...
            <div style="margin-left: auto; display: flex; gap: 0.5rem;"><button type="button" class="btn btn-secondary btn-sm texter-copy-btn texter-copy-note-btn" data-index="${i}">Copy</button><button type="button" class="btn btn-danger btn-sm texter-delete-note-btn" data-index="${i}">Delete</button></div>
          </div>
